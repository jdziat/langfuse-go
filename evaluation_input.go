...
// Any struct implementing this interface will have its fields extracted
// and flattened for direct access in LLM-as-a-Judge evaluators.
type EvalInput interface {
	// EvalFields returns the evaluation-relevant fields as a flat map.
	// This allows custom types to control how they're serialized for evaluation.
	EvalFields() map[string]any
}

// EvalOutput represents a structured output that can be flattened for evaluation.
type EvalOutput interface {
	// EvalFields returns the evaluation-relevant fields as a flat map.
	EvalFields() map[string]any
}
...
func NewInputFlattener(mode EvaluationMode) *InputFlattener {
	f := &InputFlattener{
		mode:          mode,
...
// - Maps
// - EvalInput implementations
// - Nested structures (hoisted to top level with prefix)
func (f *InputFlattener) Flatten(data any) map[string]any {
	if data == nil {
		return nil
	}

	// Check for EvalInput implementation
	if ei, ok := data.(EvalInput); ok {
...
// This is the recommended structure for evaluation-ready traces.
type StandardEvalInput struct {
	// Query is the user's question or input (maps to "input" or "user_input").
...
// EvalFields implements EvalInput.
func (s *StandardEvalInput) EvalFields() map[string]any {
	result := make(map[string]any)
...
type StandardEvalOutput struct {
...
// EvalFields implements EvalOutput.
func (s *StandardEvalOutput) EvalFields() map[string]any {
	result := make(map[string]any)
...
func prepareInputForEval(data any, config *EvaluationConfig) any {
	if config == nil || config.Mode == EvaluationModeOff {
		return data
	}
