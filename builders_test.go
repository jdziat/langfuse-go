...
func TestMetadataBuilder(t *testing.T) {
	...
	t.Run("merge overwrites existing keys", func(t *testing.T) {
		metadata := BuildMetadata().
			String("key", "original").
			Merge(Metadata{"key": "overwritten"}).
			Build()
...
func TestEndResult(t *testing.T) {
	...
	t.Run("Ok returns false when error", func(t *testing.T) {
		result := EndResult{Error: errors.New("test error")}
		if result.Ok() {
...
func TestEndOptions(t *testing.T) {
	t.Run("WithOutput sets output", func(t *testing.T) {
		cfg := &endConfig{}
		WithOutput("test output")(cfg)
		if cfg.output != "test output" {
			t.Errorf("output = %v, want test output", cfg.output)
		}
	})

	t.Run("WithUsage sets tokens", func(t *testing.T) {
		cfg := &endConfig{}
		WithUsage(100, 50)(cfg)
		if cfg.inputTokens != 100 {
			t.Errorf("inputTokens = %d, want 100", cfg.inputTokens)
		}
		if cfg.outputTokens != 50 {
			t.Errorf("outputTokens = %d, want 50", cfg.outputTokens)
		}
		if !cfg.hasUsage {
	...
	t.Run("WithError sets level and message", func(t *testing.T) {
		cfg := &endConfig{}
		testErr := errors.New("test error")
		WithError(testErr)(cfg)
		if cfg.level != ObservationLevelError {
			t.Errorf("level = %v, want ERROR", cfg.level)
		}
		if cfg.statusMessage != "test error" {
			t.Errorf("statusMessage = %s, want test error", cfg.statusMessage)
		}
		if cfg.observationError != testErr {
			t.Error("observationError should be set")
		}
	})

	t.Run("WithEndTime sets custom time", func(t *testing.T) {
		cfg := &endConfig{}
		customTime := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
		WithEndTime(customTime)(cfg)
		if !cfg.endTime.Equal(customTime) {
			t.Errorf("endTime = %v, want %v", cfg.endTime, customTime)
		}
		if !cfg.hasEndTime {
...
func TestSpanEndWith(t *testing.T) {
	...
	t.Run("EndWith with output", func(t *testing.T) {
		result := span.EndWith(ctx, WithOutput("result"))
		if !result.Ok() {
	...
	t.Run("EndWith with error", func(t *testing.T) {
		...
		result := span2.EndWith(ctx, WithError(errors.New("something failed")))
		if !result.Ok() {
			t.Errorf("EndWith failed: %v", result.Error)
		}
	})
	...
	t.Run("EndWith with multiple options", func(t *testing.T) {
		...
		result := span3.EndWith(ctx,
			WithOutput("final result"),
			WithEndMetadata(Metadata{"key": "value"}),
			WithEndLevel(ObservationLevelWarning),
			WithStatusMessage("completed with warnings"),
		)
		if !result.Ok() {
			t.Errorf("EndWith failed: %v", result.Error)
		}
...
func TestGenerationEndWith(t *testing.T) {
	...
	t.Run("EndWith with completion start time", func(t *testing.T) {
		...
		result := gen.EndWith(ctx,
			WithOutput("Streamed response"),
			WithUsage(100, 50),
			WithCompletionStart(completionStart),
		)
		if !result.Ok() {
			t.Errorf("EndWith failed: %v", result.Error)
		}
...
func TestBatchError(t *testing.T) {
	...
	t.Run("IsBatchError helper", func(t *testing.T) {
		batchErr := &BatchError{Total: 1, Succeeded: 0, Errors: map[int]error{}}
		if !IsBatchError(batchErr) {
			t.Error("IsBatchError should return true for BatchError")
		}
		if IsBatchError(errors.New("regular error")) {
	...
	t.Run("AsBatchError helper", func(t *testing.T) {
		batchErr := &BatchError{Total: 1, Succeeded: 0, Errors: map[int]error{}}
		result := AsBatchError(batchErr)
		if result != batchErr {
			t.Error("AsBatchError should return the BatchError")
		}
		if AsBatchError(errors.New("regular error")) != nil {
...
func TestBuildersIntegration(t *testing.T) {
	...
	t.Run("full workflow with builders", func(t *testing.T) {
		...
		result := gen.EndWith(ctx,
			WithOutput("Go is a programming language..."),
			WithUsage(10, 25),
			WithEndMetadata(BuildMetadata().
