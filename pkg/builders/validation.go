...
func (v *Validator) AddFieldError(field, message string) {
	v.errors = append(v.errors, pkgerrors.NewValidationError(field, message))
}
...
func ValidateID(field, value string) error {
	if value == "" {
		return pkgerrors.NewValidationError(field, "cannot be empty")
	}
...
func ValidateIDFormat(field, value string) error {
	...
	if !pkgingestion.IsValidUUID(value) {
		return pkgerrors.NewValidationError(field, "must be a valid UUID format")
	}
	return nil
}
...
func ValidateName(field, value string, maxLength int) error {
	...
	if maxLength > 0 && utf8.RuneCountInString(value) > maxLength {
		return pkgerrors.NewValidationError(field, fmt.Sprintf("exceeds maximum length of %d characters", maxLength))
	}
	return nil
}
...
func ValidateRequired(field, value string) error {
	if value == "" {
		return pkgerrors.NewValidationError(field, "is required")
	}
	return nil
}
...
func ValidatePositive(field string, value int) error {
	if value < 0 {
		return pkgerrors.NewValidationError(field, "must be non-negative")
	}
	return nil
}
...
func ValidateRange(field string, value, min, max int) error {
	if value < min || value > max {
		return pkgerrors.NewValidationError(field, fmt.Sprintf("must be between %d and %d", min, max))
	}
	return nil
}
...
func ValidateMetadata(field string, metadata Metadata) error {
	...
	for key := range metadata {
		if key == "" {
			return pkgerrors.NewValidationError(field, "metadata keys cannot be empty")
		}
	}
	return nil
}
...
func ValidateTags(field string, tags []string) error {
	...
	for i, tag := range tags {
		if tag == "" {
			return pkgerrors.NewValidationError(field, fmt.Sprintf("tag at index %d cannot be empty", i))
		}
	}
	return nil
}
...
func ValidateLevel(field string, level types.ObservationLevel) error {
	switch level {
	case "", types.ObservationLevelDebug, types.ObservationLevelDefault, types.ObservationLevelWarning, types.ObservationLevelError:
		return nil
	default:
		return pkgerrors.NewValidationError(field, fmt.Sprintf("invalid level: %s", level))
	}
}
...
func ValidateScoreValue(field string, value float64, min, max float64) error {
	if value < min || value > max {
		return pkgerrors.NewValidationError(field, fmt.Sprintf("must be between %.2f and %.2f", min, max))
	}
	return nil
}

// ValidateDataType validates a score data type.
func ValidateDataType(field string, dataType types.ScoreDataType) error {
	switch dataType {
	case "", types.ScoreDataTypeNumeric, types.ScoreDataTypeCategorical, types.ScoreDataTypeBoolean:
		return nil
	default:
		return pkgerrors.NewValidationError(field, fmt.Sprintf("invalid data type: %s", dataType))
	}
}
