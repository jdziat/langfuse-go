...
func TestHighThroughput(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/ingestion" {
			...
			if err := json.NewDecoder(r.Body).Decode(&req); err == nil {
				receivedEvents.Add(int64(len(req.Batch)))
			}
	...
	deliveryRate := float64(received) / float64(createdCount) * 100

	t.Logf("Delivery: %d/%d (%.1f%%)", received, createdCount, deliveryRate)

	// We expect at least 95% delivery under high load
	if deliveryRate < 95.0 {
		t.Errorf("Expected at least 95%% delivery, got %.1f%%", deliveryRate)
	}
}
