...
func TestMain(m *testing.M) {
	goleak.VerifyTestMain(m,
		...
		// Ignore HTTP/1.1 transport dial goroutines (connection setup/pooling)
		goleak.IgnoreAnyFunction("net/http.(*Transport).dialConn"),
...
func TestClientShutdown_WithPendingEvents(t *testing.T) {
	...
	client, err := langfuse.New(
		...
		langfuse.WithFlushInterval(1*time.Hour), // Don't auto-flush
		langfuse.WithTimeout(5*time.Second),
		langfuse.WithShutdownTimeout(10*time.Second),
	)
	if err != nil {
...
func TestClientShutdown_Timeout(t *testing.T) {
	...
	client, err := langfuse.New(
		...
		"sk-lf-test-key",
		langfuse.WithBaseURL(server.URL),
		langfuse.WithBatchSize(1),
		langfuse.WithFlushInterval(1*time.Hour),
		langfuse.WithTimeout(1*time.Second),
		langfuse.WithShutdownTimeout(2*time.Second),
	)
	if err != nil {
		t.Fatalf("New failed: %v", err)
	}
